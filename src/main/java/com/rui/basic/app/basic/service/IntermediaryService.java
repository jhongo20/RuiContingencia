...
public class IntermediaryService {
    ...
    /**
 * Guarda un nuevo intermediario
 * 
 * @param intermediary El intermediario a guardar
 * @return El intermediario guardado
 */
@Transactional
public RuiIntermediary save(RuiIntermediary intermediary) {
    try {
        log.debug("Guardando nuevo intermediario");
        
        // Validar campos obligatorios según el tipo
        validateRequiredFields(intermediary);
        
        // Guardar primero persona o compañía según corresponda
        ...
        // Guardar infraestructura humana si existe
        if (intermediary.getInfrastructureHumanId() != null && 
            intermediary.getInfrastructureHumanId().getLawyerId() != null &&
            intermediary.getInfrastructureHumanId().getLawyerId().getDocumentNumber() != null) {
        ...
        // Guardar infraestructura operativa si existe
        if (intermediary.getInfrastructureOperationalId() != null) {
            RuiInfraOperational infraOp = intermediary.getInfrastructureOperationalId();
            // No es necesario guardar aquí, se guardará con cascada
        }
        
        // Guardar el intermediario
        return intermediaryRepository.save(intermediary);
    } catch (Exception e) {
        log.error("Error al guardar intermediario: {}", e.getMessage(), e);
        throw new BusinessException("Error al guardar intermediario: " + e.getMessage(), e);
    }
}

/**
 * Valida los campos obligatorios según el tipo de intermediario
 */
private void validateRequiredFields(RuiIntermediary intermediary) {
    if (intermediary.getTypeIntermediarieId() == null) {
        throw new BusinessException("El tipo de intermediario es obligatorio");
    }
    
    // Validar datos según el tipo (agente o empresa)
    if (intermediary.getTypeIntermediarieId().getId() == 4L) {
        // Es un agente (persona)
        if (intermediary.getPersonId() == null) {
            throw new BusinessException("Los datos de la persona son obligatorios para agentes");
        }
        RuiPerson person = intermediary.getPersonId();
        if (StringUtils.isEmpty(person.getDocumentNumber())) {
        ...
        if (StringUtils.isEmpty(person.getFirstSurname())) {
        ...
        RuiCompany company = intermediary.getCompanyId();
        if (StringUtils.isEmpty(company.getNit())) {
...
/**
 * Obtiene el último secuencial usado para el número de radicado en un mes determinado
 * 
 * @param pattern Patrón a buscar (por ejemplo "RIND-2025-03-%")
 * @return El último secuencial usado (0 si no hay ninguno)
 */
public Long getLastSequentialForMonth(String pattern) {
    try {
        Long lastSequential = intermediaryRepository.findLastSequentialByPattern(pattern);
        return lastSequential != null ? lastSequential : 0L;
    } catch (Exception e) {
        log.error("Error al obtener último secuencial: {}", e.getMessage(), e);
        // En caso de error, retornar 0 para evitar bloquear la aplicación
        return 0L;
    }
}

/**
 * Crea un historial inicial para un intermediario
 * 
 * @param intermediaryId ID del intermediario
 * @param observation Observación
 * @param currentUser Usuario actual
 */
@Transactional
public void createInitialHistory(Long intermediaryId, String observation, RuiUser currentUser) {
    try {
        RuiIntermediary intermediary = findById(intermediaryId);
        ...
        history.setDatetime(new Date());
        history.setStatus((short) 1);
        
        intermediaryHistoryRepository.save(history);
    } catch (Exception e) {
        log.error("Error al crear historial inicial: {}", e.getMessage(), e);
        throw new BusinessException("Error al crear historial inicial: " + e.getMessage(), e);
    }
}
}
