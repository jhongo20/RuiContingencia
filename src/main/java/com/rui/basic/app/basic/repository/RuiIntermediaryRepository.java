...
@Query(value = """
   ...
   Page<RuiIntermediary> findAllWithDetails(Pageable pageable);
    ...
    // Agregar al RuiIntermediaryRepository.java
/**
 * Encuentra el último secuencial usado en un patrón de número de radicado
 * 
 * @param pattern El patrón a buscar (por ejemplo "RIND-2025-03-%")
 * @return El último secuencial usado
 */
@Query(value = "SELECT NVL(MAX(TO_NUMBER(SUBSTR(RADICATE_NUMBER, INSTR(RADICATE_NUMBER, '-', 1, 3) + 1))), 0) " +
               "FROM RUI_INTERMEDIARIES " +
               "WHERE RADICATE_NUMBER LIKE ?1", nativeQuery = true)
Long findLastSequentialByPattern(String pattern);
